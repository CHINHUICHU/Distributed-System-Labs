...
- The paper's Section 5.2 mentions election timeouts in the range of 150 to 300 milliseconds. Such a range only makes sense if the leader sends heartbeats considerably more often than once per 150 milliseconds (e.g., once per 10 milliseconds). Because the tester limits you to tens of heartbeats per second, you will have to use an election timeout larger than the paper's 150 to 300 milliseconds, but not too large, because then you may fail to elect a leader within five seconds.
...
- Do yourself a favor for future labs and write (or re-write) code that's clean and clear. For ideas, re-visit the Guidance page with tips on how to develop and debug your code.
...
The `ok 6.5840/raft 35.557s` means that Go measured the time taken for the 3B tests to be 35.557 seconds of real (wall-clock) time. The `user 0m2.556s` means that the code consumed 2.556 seconds of CPU time. If your solution uses much more than a minute of real time for the 3B tests, or much more than 5 seconds of CPU time, you may run into trouble later on. Look for time spent sleeping or waiting for RPC timeouts, loops that run without sleeping or waiting for conditions or channel messages, or large numbers of RPCs sent.
...
Then the leader's logic can be:
...
- Your code should pass all the 3C tests, as well as the 3A and 3B tests.
...
- Run `git pull` to make sure you have the latest software.
...
- Your code should pass all the 3D tests, as well as the 3A, 3B, and 3C tests.
